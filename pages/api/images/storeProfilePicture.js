...
handler.post(async (req, res) => {
  ...
  await new Promise((resolve, reject) => {
    upload.single("image")(req, res, async (err) => {
      ...
      const outputPath = join(file.destination,'profilepicture.' + imageMetadata.format);
      await sharp(file.path).resize(1000, 1000, { fit: 'inside' }).toFile(outputPath);
