...
export default function AddNewCollection({ collections, photographer }) {
    ...
    async function handleCreateNewCollections(e) {
        ...
        formData.append('photographerID', photographer.personID);
        ...
        <div className={`relative`}>
            {activeView && (
                <div className={`absolute w-full h-full flex justify-center items-center z-10 transform transition-all duration-500 ease-out ${activeView ? "backdrop-blur-sm opacity-100 scale-100" : "opacity-0 scale-0"}`}>
                    <Updatecollection collections={collections} setActiveView={setActiveView} id={activeView} photographers={photographer} />
                </div>
            )}
            <ErrorBoundary>
