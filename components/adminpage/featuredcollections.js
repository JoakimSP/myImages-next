...
export default function Featuredcollections({ featuredcol }) {
    ...
    async function handleUpdateFeaturedColHompageText(e){
        e.preventDefault();
        ...
        try {
            const response = await fetch('/api/application/collections/updateFeaturedCollectionText', {
                method: 'POST',
                headers: {
    ...
    return (
        <div className="flex-1 bg-gray-800 shadow-xl rounded-xl px-10 py-6">
                ...
                <ul>
                    {currentCol?.map((collection) => (
                        <li key={collection.id} className="flex items-center justify-between mb-2 text-lg text-gray-400 hover:text-gray-300 transition-all duration-200">
                            ...
                            <span className="text-white">sort order: {collection.sortOrder}</span>
            ...
            <h2 className="text-3xl text-white font-bold mb-6">Edit title and subtitle above featured collection slider at homepage</h2>
            <div className="flex-1 bg-gray-800 shadow-xl rounded-xl px-10 py-6">
                <form onSubmit={handleUpdateFeaturedColHompageText}>
                    <InputField key={"title"} label={"title"} name={"title"} textColor={"white"} type="text" placeholder={featuredcol.title} />
                    <InputField key={"subTitle"} label={"subTitle"} name={"subTitle"} textColor={"white"} type="text" placeholder={featuredcol.subTitle} />
                    <button type="submit" className="py-2 px-4 font-semibold rounded-lg shadow-md text-white bg-blue-500 hover:bg-blue-700 transition duration-300 ease-in-out">Update</button>
                </form>
            </div>
        </div>
    )
