...
export default function ShowPhotographerCollection({ photographer }) {
    ...
    const [loadingId, setLoadingId] = useState(null); // State to track the loading collection

    const handleClick = (id) => {
        setLoadingId(id); // Set the loading state to the clicked collection's ID
        // You might want to navigate programmatically after setting the loading state, depending on your use case
    };

    return (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            {collection.map((col) => {
                const isLoading = col.id === loadingId; // Check if this collection is loading
